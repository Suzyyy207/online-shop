...
| 接口名                  | 方法 | 功能             |
| ----------------------- | ---- | ---------------- |
| getShopAvatarByShopname | POST | 获取指定商店头像 |
...
| ------ | ------------ | ----------------- |
| state  | 是否添加成功 |   |
| data  | 商品列表 | 每个商品包括goodsAvatar列表，favorites等  |
...
- 提交删除申请时：
  ...
  **本次不要求**~~-- 后端检查***商店是否有订单未完成***，如果有则不允许提交申请，并且在message中提示~~
  ~~-- 如果没有未完成的订单，则申请提交成功，管理员页面会出现该店铺的删除申请~~
- 商户提交删除申请后：
  -- 商户无法再对商品进行操作（包括商品注册、商品下架等等）
  -- 在“我的店铺”的店铺信息板块，显示商店当前**处于“删除审核”状态**
  -- 原来的删除按钮失效
  -- 提供“撤销删除申请”的按钮
  -- 主页无法看到店铺信息
...
| -------- | ------ | --------- |
| shopname | 店名 |           |
| cancelType | 类型 |  与is_admitted同步<br>0/2:撤销注册申请<br>3/4：撤销删除申请       |
...
| -------- | ------ | --------- |
| shopname | 店名 |           |
| is_admitted | 类型 |  与is_admitted同步<br>0:同意注册申请<br>3：同意删除申请         |
